# -*- coding: utf-8 -*-
"""
Created on Fri May 31 10:26:52 2024
...
We need to change mean and standard deviation for correlated version! 
...
class Subproblem():
    ...
    def mean_correlation(self, value):
        coeff= self.coeff*value   #coefficient values for regression model which is affected by mean value of travel time without out correlation 
        mean_correlated_travel_time= value + np.dot(coeff,self.prob)
        return mean_correlated_travel_time
    
    def std_correlation(self,value):
        coeff=(self.coeff*value)**2
        variance_correlated_travel_time= (value*self.TCov)**2+np.sum(coeff)*(self.prob[0]*(1-self.prob[0]))   # it needs to be modified
        std=np.sqrt(variance_correlated_travel_time)
        return std
    ...
    def arrival_approximation(self,sequence,tasks,d):  #all_visit_time
        ...
        for i in range(1,num_node+1):
            ...
            if i == 1:
                ...
                mean_arrival[i-1]=visit_time[i-1]+self.mean_correlation(self.instance.distance_matrix[from_i][to_j])
                
                std_arrival[i-1]=self.std_correlation(self.instance.distance_matrix[from_i][to_j])
                ...
                mean_arrival[i-1]=mean_start[i-2]+visit_duration_mean_i+self.mean_correlation(self.instance.distance_matrix[from_i][to_j])
                
                variance=std_start[i-2]+visit_duration_std_i**2+(self.std_correlation(self.instance.distance_matrix[from_i][to_j]))**2
                std_arrival[i-1]=np.sqrt(variance)
