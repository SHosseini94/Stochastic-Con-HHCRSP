# -*- coding: utf-8 -*-
"""
Created on Fri May 31 11:26:06 2024
...
from Subproblem_correlated import Subproblem
        ...
        for k in model._activeAides:
            if Solve[k.id]==1:
                ...
                if value not in model._instance.list_feasible_per_aide[k.id] and new_patient:
                    ...
                    #print(list_Pik[k.id])
                    subproblem=Subproblem(model._instance,k.id,list_Pik[k.id],model._maximum_delay,model._overtime, model._alpha,model._TCov,model._SCov,model._coeff,model._prob)
                    model._subcall+=1
                    cut=[]
...
class Master_Problem():
    ...
    def __init__(self,instance,l,overtime,alpha,TCov,SCov,coeff,prob):
    ...
    def branch_and_check(self,env):
        ...
        with Model(env=env) as m:
            ...
            m.addConstrs((y[k,i,d]==self.fixed_patient[i].pre_assigned_day[d] for d in range(self.horizon) for k,i 
            ...
            if m.status == GRB.OPTIMAL:
                ...
                for k in self.active_aide:
                    ...
                    if Solve[k.id]==1:
                        ...
                        if new_patient:
                            subproblem=Subproblem(self.instance,k.id,f[k.id],self.maximum_delay,self.overtime,self.alpha,self.TCov,self.SCov,self.coeff,self.prob)
                            cut=[]
    ...
    def write_solution(self,obj,nbcall,nblazy,subcall,complete_sub,inf_sub,time_limit,comp_time,sub_time):
        ...
        with open("output_approx_correlated.txt","a") as f:
            f.write("----------------------------------------------------------------\n")
        ...
        #"""        
        with open("route_approx_correlated.txt","a") as f:
            f.write("----------------------------------------------------------------\n")
            f.write(self.instance.instance_name)
            f.write('\n')
            f.write("Method ArrivalApproximation \n")
        ...
        with open("schedule_approx_correlated.txt","a") as f:
            f.write("----------------------------------------------------------------\n")
            f.write(self.instance.instance_name)
            f.write('\n')
            f.write("Method ArrivalApproximation \n")
        ...
        with open("Route-Schedule_approx_correlated.txt","a") as f:
