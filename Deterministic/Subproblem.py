...
class Subproblem():
    ...
    def __init__(self,instance,aide_id,list_Pik_per_day,l,overtime,alpha):
    ...
    def generate_cut(self,list_feasible_day_per_aide,list_cuts,final_call):
        ...
        """
        self.time_window=[integer_var(min=self.instance.list_patient[i].time_window[0],
                                        max=self.instance.list_patient[i].time_window[1]-self.instance.list_patient[i].visit_duration
                                        ,name="time_window_patient_{}".format(i)) for i in self.union_patients]
        """
        ...
        #self.time_window_start=integer_var(min=self.instance.list_aide[self.aide_id].time_window_start[0],
         #                               max=self.instance.list_aide[self.aide_id].time_window_start[1],name="time_window_start")
        ...
        for d in range(self.number_pattern):
            ...
            #for i in range(1,self.nb_points[d]-1):
                ...
                #self.sub.add(any([visit_time[i]==start_of(tasks[j]) for j in range(1,self.nb_points[d]-1)]))
            ...
            visit_time=integer_var_list(size=self.nb_points[d],name="visit_time_pattern_{}".format(d+1))
            self.visit_time[d]=visit_time
            
            
            locations=[]
            for i in range(self.nb_points[d]):
                if i==0:
                    locations.append(self.instance.list_aide[self.aide_id].location_start)
                elif i==self.nb_points[d]-1:
                    locations.append(self.instance.list_aide[self.aide_id].location_end)
                else:
                    locations.append(self.assignment_pattern[d][i-1]+self.instance.number_aide)
            route=integer_var_list(size=self.nb_points[d],domain=locations,name="route_pattern_{}".format(d+1))
            self.route[d]=route
            
            
            self.sub.add(all_diff(route))
            self.get_route_values(d)
            
            
            
            for v in range(self.nb_points[d]):
                if v==0:
                    start=integer_var(name="start_pattern{}_node{}".format(d+1,v+1))
                    self.start[d].append(start)
                    self.sub.add(start==visit_time[v])
                else:
                    self.check_subroute_success(d,v)
            
               
            #changed 
            """
          
            #arrival_statistics=CpoBlackboxFunction(impl=self.scenario_success,dimension=self.nb_points[d]-1)
            ...
            #self.time_itv[d]=[]
            """
            for i in self.assignment_pattern[d]:
                
                #mean,std=arrival_statistics(self.sequence[d],self.visit_time[d],d,v)
                
                mean,std=arrival_statistics(self.sequence[d],d,i)
                
                self.sub.add(mean+2*std<= self.time_window[i]+self.L)
                    
                    #self.sub.add(mean+2*std<= start_of(self.sequence[d][v])+self.L)
                #else:  #only allow to work maximum for overtime longer than their end time window on each day 
                 #   self.sub.add(mean+2*std<= self.time_window_end+self.overtime)
            """
            
            #"""  
            
            
            #success_rate=arrival_statistics(seq,tasks,d)
            
            #for v in range(1,self.nb_points[d]):
                
                #self.sub.add(success_rate[v-1]>=self.alpha)
        ...
        #"""
            ...
            for v in range(1,self.nb_points[d]-1):
        ...
        #file=open('CPOModel_deterministic.txt','a')
        #self.sub.export_model(file)
        
        sol=self.sub.solve(LogVerbosity="Quiet",Workers=1)  #Workers=4 TimeLimit=10 log_output=True LogVerbosity="Quiet"
