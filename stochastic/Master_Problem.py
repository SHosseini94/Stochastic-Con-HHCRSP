           ...
           for k in model._activeAides:
               if Solve[k.id]==1:
                   ...
                   if value not in model._instance.list_feasible_per_aide[k.id] and new_patient:
                       ...
                       #print(list_Pik[k.id])
                       subproblem=Subproblem(model._instance,k.id,list_Pik[k.id],model._maximum_delay,model._overtime, model._alpha,model._scenario)
                       model._subcall+=1
                       cut=[]
...
# # Master_Problem Class.
# 
# This class solves the master problem. 
...
class Master_Problem():
    ...
    def __init__(self,instance,scenario,l,overtime,alpha):
    ...
    def branch_and_check(self,env):
        ...
        with Model(env=env)as m:
            ...
            #m.setParam("LogFile","log_file_master_scenario.txt")
            #m.write("heching-master_scenario.lp")
            ...
            if m.status == GRB.OPTIMAL:
                ...
                for k in self.active_aide:
                    ...
                    if Solve[k.id]==1:
                        ...
                        if new_patient:
                            subproblem=Subproblem(self.instance,k.id,f[k.id],self.maximum_delay,self.overtime,self.alpha,m._scenario)
                            cut=[]
    ...
    def write_solution(self,obj,nbcall,nblazy,subcall,complete_sub,inf_sub,time_limit,comp_time,sub_time):
        ...
        with open("output_scenario.txt","a") as f:
            f.write("----------------------------------------------------------------\n")
            f.write(self.instance.instance_name)
            f.write('\n')
            f.write("Method Scenario \n")
            f.write("ScenarioDistribution " + self.scenario.distribution)
            f.write('\n')
            f.write("NewPatients {}\n".format(len(self.free_patient)))
            f.write("ActiveAides {}\n".format(len(self.active_aide)))
            f.write("Scenario {}\n".format(len(self.scenario.travel_time)))
        ...
        with open("change_scenario.txt","a") as f:
            f.write("----------------------------------------------------------------\n")
            f.write(self.instance.instance_name+'\n')
        ...
        with open("route_scenario.txt","a") as f:
            
            f.write("----------------------------------------------------------------\n")
            f.write(self.instance.instance_name)
            f.write('\n')
            f.write("Method Sceanrio \n")
        ...
        with open("schedule_scenario.txt","a") as f:
        ...
        with open("Route-Schedule_scenario.txt","a") as f:
        ...
        with open("route.txt","a") as f:
            ...
            for i in range(self.nb_aide):
                f.write('aide_{}'.format(i+1))
                f.write('\n')
                for d in range(self.horizon):
                    f.write('day_{}'.format(d+1))
                    f.write('\n')
                    f.write(str(self.instance.list_aide[i].route[d]))
                    f.write('\n')
